...
void CalculationManager::InitializeBeagleInstance(int nTips, int nClas, int nHolders, int nstates, int nchar, int nrates, SequenceData *data){
	assert(useBeagle);
...
#ifdef BEAGLEPART
	//beagleInstances.push_back(beagleInstNum);
#else
	...
	SendTipDataToBeagle(beagleInstNum, data);

	outman.UserMessage("#######################################################");
	}
*/
/*
void CalculationManager::SendTipDataToBeagle(int beagleInstNum, SequenceData *data){
	...
	for (int t = 0; t < data->NTax(); t++) {
		...
		if (partialAmbig) {
			...
			CheckBeagleReturnValue(
				beagleSetTipPartials(beagleInstNum, t, &(tipPartial[0])),
				"beagleSetTipStates");
		}
		else {
			...
			CheckBeagleReturnValue(
				beagleSetTipStates(beagleInstNum, t, &(dat[0])),
				"beagleSetTipStates");
		}
	}
...
//don't think we need this at the CalcMan level, just SubsetMan
#ifdef TO_DELETE
...
#endif
		//beagleSetTransitionMatrix(PmatIndexForBeagle(theIndex), **thePMat, 1.0);
...
#endif
			//beagleSetTransitionMatrix(D1MatIndexForBeagle(theIndex), **theD1Mat, 0.0);
			//beagleSetTransitionMatrix(D2MatIndexForBeagle(theIndex), **theD2Mat, 0.0);
...
void CalculationManager::PerformClaOperationBatch(int beagleInst, const list<ClaOperation> &theOps) {
	...
	//BEAGLEPART TODO - loop over subsets here, operations will be same
	for (vector<SubsetCalculationManager *>::iterator subit = subsetManagers.begin(); subit != subsetManagers.end(); subit++) {
		CheckBeagleReturnValue(
			beagleUpdatePartials(
				(*subit)->BeagleInstance(),
...
int CalculationManager::ScalerIndexForBeagle(int ind) const {
...
int CalculationManager::PartialIndexForBeagle(int ind) const {
